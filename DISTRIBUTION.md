...
2. 앱과 공유 확장의 버전 및 빌드 번호를 함께 증가시킵니다.
...
- 큰 이미지(40 MP 상한)는 내용이 압축되지 않는 최악 입력에서 앱과 격리 디코더를 합쳐
  약 1 GB에 가까운 메모리를 쓸 수 있습니다. 일반 사진과 8 MP급 이미지는 훨씬 적게 씁니다.
- `open -a MetaShield <파일>` 또는 서비스 메뉴로 호출하면 창 없이 즉시 처리됩니다.
  Finder 서비스를 위한 의도된 동작이지만, 실행 즉시 원본이 바뀐다는 점을 명시합니다.
...
4. 첫 실행에서 `사진 앱 권한 연결…`을 누르고 시스템 공유 창의 `MetaShield`를 선택한 뒤 사진
   추가 권한을 허용합니다. 설정 이미지는 보관함에 추가되지 않습니다. Finder 서비스와 공유
   확장의 등록 자체는 자동입니다. `다음으로 편집` 등 호스트 앱 경로는 별도 TCC 주체라 처음
   사용할 때 권한을 한 번 더 물을 수 있습니다. Developer ID가 없는 새 빌드는 코드 요구사항이
   달라져 업데이트 뒤 권한을 다시 요청할 수 있습니다.
...
- 구버전 교체 업그레이드와 레거시 빠른 동작 자동 정리
- Finder 서비스 단일·다중 선택, 한글·공백·따옴표 파일명
- 사진 권한 허용·거부·나중에 변경
- 인터넷을 끈 상태의 이미지 처리
...
업데이트는 새 DMG의 앱을 `/Applications/MetaShield.app` 위에 교체합니다. 새 빌드는 다시
`확인 없이 열기`와 사진 추가 권한이 필요할 수 있습니다.

제거할 때는 `/Applications/MetaShield.app`을 휴지통으로 옮깁니다. Finder 서비스와 사진 공유
확장은 앱 번들에 포함되며 별도 workflow·LaunchAgent·로그인 항목을 설치하지 않습니다.
이미 만든 PNG와 사진 보관함 항목은 자동 삭제되지 않습니다.

휴지통 이동 뒤 LaunchServices가 Finder 서비스와 공유 확장을 내리는 것은 확인해야 합니다.
Gatekeeper 승인·TCC 권한·최근 실행 기록·환경설정은 macOS 소유 상태라 앱 삭제와 동시에
사라진다고 보장할 수 없지만, 실행 코드나 백그라운드 프로세스는 남지 않아야 합니다. 공유 확장
컨테이너 `~/Library/Containers/kr.metashield.app.share`가 macOS 보호로 남을 수 있으나 처리한
이미지를 영구 보관하는 위치는 아니며, 보호를 우회한 수동 삭제를 안내하지 않습니다.
